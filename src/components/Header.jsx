...
const Header = () => {
  ...
  return (
    <div className='min-h-screen start1920:h-800  flex justify-center items-center bg-header bg-cover bg-center flex-col  relative pt-[10px]'>
      ...
      <div className='absolute top-0 right-0 hidden sm:block'> <img src={top_layer} alt="layer" /></div>
      <div className='absolute bottom-[112px]  left-0 max-w-[243px] hidden sm:block'> <img src={left_layer} alt="layer" /></div>
      <div className='container max-w-[1140px] mx-auto px-3 flex justify-between z-[1]'>
        ...
        <button
          onClick={handleClick}
          className="sm:hidden z-10 relative"
        >
          {nav ? "Close" : "Menu"}
        </button>
        <div className={`flex items-center gap-5 flex-col sm:flex-row        max-sm:min-h-screen max-sm:top-0 max-sm:left-0 max-sm:w-screen max-sm:justify-center max-sm:items-center max-sm:bg-blue-500   max-sm:fixed transition-transform duration-500 ${nav ? "max-sm:-translate-y-0" : "max-sm:-translate-y-full"
          }`}>
          <button onClick={handleClick} className='font-oswald font-bold text-base text-center text-[#39C6FD] hover:text-[#070D37] hover:bg-[#39C6FD] py-2 px-5 rounded border-[#39c6fd] border-[1px] transition-all duration-500'>LOGIN</button>
          <button onClick={handleClick} className='font-oswald font-bold text-base text-center text-[#39C6FD] hover:text-[#070D37] hover:bg-[#39C6FD] py-2 px-5 rounded border-[#39c6fd] border-[1px] transition-all duration-500'>SIGN UP</button>
        </div>
      </div>
      <div className='flex-grow flex justify-center items-center w-full sm:z-10'>
